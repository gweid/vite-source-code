...
// 在 index.ts 中会执行，执行这个就是启动服务端的 socket
export const hmrPlugin: ServerPlugin = ({
  root,
  app,
  server,
  watcher, // 由 index.ts 中的 chokidar.watch 得到
  resolver,
  config
}) => {
  app.use((ctx, next) => {
  ...
  watcher.on('change', (file) => {
    if (!(file.endsWith('.vue') || isCSSRequest(file))) {
      ...
      //除了普通的.css之外，所有内容都被视为HMR依赖项。
      //普通css在./serverPluginCss.ts中有自己的HMR逻辑。
      handleJSReload(file)
    }
  })
}
