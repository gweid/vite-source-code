...
export const vuePlugin: ServerPlugin = ({
  ...
  const handleVueReload = (watcher.handleVueReload = async (
    ...
    // 如果修改了 vue 文件，那么 isEqualBlock 执行会为 false，此时将 needRerender 置为 true
    ...
    // 通过 send() 方法向浏览器推送 type 为 vue-rerender 以及携带修改的文件路径的信息
    // 然后就是在 客户端 client.ts 中监听 message 的地方就会拿到对应的 data，再通过 import 发起获取该模块的请求
    if (needRerender) {
      send({
        type: 'vue-rerender',
        path: publicPath,
  ...
  // watcher 监听 vue 文件的改变，然后通过 handleVueReload 控制服务端 socket 推送什么更新类型
